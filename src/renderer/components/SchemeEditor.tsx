...
export function SchemeEditor({ company, scheme, companies, onSave, onClose }: SchemeEditorProps) {
  ...
  const schemeForPreview: Scheme = { ...scheme, pattern, tokens }
  const livePreview = (() => {
    try {
      return renderPattern(pattern, previewValues, 'wav', schemeForPreview)
    } catch {
  ...
  const handleParse = () => {
    const result = parseFilename(testParseInput, schemeForPreview)
  ...
  const handleSave = () => {
    ...
    const next = companies.map((c) =>
      c.id === company.id
        ? { ...c, schemes: c.schemes.map((s) => (s.id === scheme.id ? updatedScheme : s)) }
        : c
    )
    onSave(next)
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-lg bg-white shadow-xl dark:bg-zinc-800">
        <div className="flex items-center justify-between border-b border-zinc-200 p-4 dark:border-zinc-700">
          <h2 className="text-lg font-semibold">Edit scheme</h2>
